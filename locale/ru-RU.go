...
var russianRussianFederation = Formatters{
	-1 << 63:                    func(d time.Duration) string { return fmt.Sprintf("через %.0f лет", math.Ceil(-d.Hours()/(24.0*30*12))) },
	-50 * (24 * time.Hour) * 30: func(d time.Duration) string { return fmt.Sprintf("через %.0f года", math.Ceil(-d.Hours()/(24.0*30*12))) },
	-17 * (24 * time.Hour) * 30: func(_ time.Duration) string { return "через год" },
	-10 * (24 * time.Hour) * 30: func(d time.Duration) string { return fmt.Sprintf("через %.0f месяцев", math.Ceil(-d.Hours()/(24.0*30))) },
	-4 * (24 * time.Hour) * 30:  func(d time.Duration) string { return fmt.Sprintf("через %.0f месяца", math.Ceil(-d.Hours()/(24.0*30))) },
	-45 * (24 * time.Hour):      func(d time.Duration) string { return "через месяц" },
	-25 * (24 * time.Hour):      func(d time.Duration) string { return fmt.Sprintf("через %.0f дней", math.Ceil(-d.Hours()/24.0)) },
	...
	-44 * time.Minute:           func(d time.Duration) string { return fmt.Sprintf("через %.0f минуты", math.Ceil(-d.Minutes())) },
	-21 * time.Minute:           func(d time.Duration) string { return fmt.Sprintf("через %.0f минут", math.Ceil(-d.Minutes())) },
	-4 * time.Minute:            func(d time.Duration) string { return fmt.Sprintf("через %.0f минуты", math.Ceil(-d.Minutes())) },
	...
	4 * time.Minute:            func(d time.Duration) string { return fmt.Sprintf("%.0f минуты назад", math.Ceil(d.Minutes())) },
	...
	25 * (24 * time.Hour):      func(d time.Duration) string { return fmt.Sprintf("%.0f дней назад", math.Ceil(d.Hours()/24.0)) },
	45 * (24 * time.Hour):      func(d time.Duration) string { return "месяц назад" },
	4 * (24 * time.Hour) * 30:  func(d time.Duration) string { return fmt.Sprintf("%.0f месяца назад", math.Ceil(d.Hours()/(24.0*30))) },
	10 * (24 * time.Hour) * 30: func(d time.Duration) string { return fmt.Sprintf("%.0f месяцев назад", math.Ceil(d.Hours()/(24.0*30))) },
	17 * (24 * time.Hour) * 30: func(_ time.Duration) string { return "год назад" },
	34 * (24 * time.Hour) * 30: func(d time.Duration) string { return fmt.Sprintf("%.0f года назад", math.Ceil(d.Hours()/(24.0*30*12))) },
	1<<63 - 1:                  func(d time.Duration) string { return fmt.Sprintf("%.0f лет назад", math.Ceil(d.Hours()/(24.0*30*12))) },
}
